...
const steps = [
  { icon: <img src={MARK} alt="" />, number: '01', title: 'Tell HHH about you', copy: 'Complete our secure eligibility form. Your application goes to Holistic Health Hub first, including when you arrive through a pharmacy-specific link.' },
  { icon: <HeartHandshake />, number: '02', title: 'HHH reviews and refers', copy: 'Our team reviews your application, follows up with you where needed and confirms the right pharmacy before activating your referral.' },
  { icon: <Stethoscope />, number: '03', title: 'Meet your specialist', copy: 'A doctor who specialises in your condition assesses you. Any treatment decision is made clinically after an appropriate consultation.' },
  { icon: <PackageCheck />, number: '04', title: 'Pharmacy care begins', copy: 'Once HHH has referred you, your pharmacy receives the activated patient record and supports payment, supply, delivery or collection.' },
];

function HomePage() {
  return <PageShell><main id="main-content">
    <section className="hhh-hero hhh-reveal"><div className="hhh-section-inner hhh-hero__inner"><div className="hhh-hero__copy"><p className="hhh-kicker">Specialist care, closer to home</p><h1>Feel heard.<br />Find a way forward.</h1><p>Explore whether cannabis-based medicines could be appropriate for you, with specialist clinical care and support from a participating community pharmacy.</p><div className="hhh-hero__actions"><PublicLink href="/eligibility" className="hhh-button hhh-button--rust">Check your eligibility <ArrowRight /></PublicLink><PublicLink href="/how-it-works" className="hhh-text-link">See how it works <ChevronRight /></PublicLink></div><div className="hhh-hero__assurance"><span><ShieldCheck /> Private and secure</span><span><Stethoscope /> Specialist-led</span><span><HeartHandshake /> Pharmacy connected</span></div></div><div className="hhh-hero__media"><img src={HERO_IMAGE} alt="A pharmacist listening to a patient in a private consultation room" fetchPriority="high" /><div><span><HeartPulse /></span><p><strong>Care built around you</strong><small>From first questions to ongoing support</small></p></div></div></div></section>
    <section className="hhh-steps"><div className="hhh-section-inner"><p className="hhh-kicker">A clear route to care</p><h2>How it works in four simple steps</h2><div className="hhh-steps__grid">{steps.map(step => <article key={step.number}><div className="hhh-step-icon">{step.icon}</div><h3><span>{step.number}</span> {step.title}</h3><p>{step.copy}</p></article>)}</div></div></section>
...
const conditionGroups = [
  { title: 'Pain', icon: <Activity />, items: ['Arthritis','Back pain','Cancer-related pain','Chronic pain','Cluster headache','Ehlers-Danlos syndromes','Endometriosis','Fibromyalgia','Migraine','Musculoskeletal pain','Neuropathic pain','Sciatica'] },
  { title: 'Neurological', icon: <Brain />, items: ['Autistic spectrum disorder','Epilepsy','Multiple sclerosis','Parkinson’s disease','Tourette’s syndrome','Trigeminal neuralgia'] },
  ...
  { title: 'Other conditions', icon: <ShieldPlus />, items: ['Cancer-related appetite loss','Crohn’s disease','Eating disorders','Palliative care','Rare skin conditions','Ulcerative colitis'] },
];
...
function PricingPage() {
  return <PageShell><main id="main-content"><section className="hhh-pricing-head hhh-section-inner"><p className="hhh-kicker">Simple, transparent pricing</p><h1>Personalised care,<br />without the guesswork.</h1><p>All appointments are with a consultant physician who specialises in your condition.</p><div className="hhh-price-grid">{[['Initial consultation','£30'],['Follow-up consultation','£30'],['Quarterly check-up','£30']].map(([label,price], index) => <article key={label}><span>0{index + 1}</span><h2>{label}</h2><strong>{price}</strong><small>Consultation fee</small></article>)}</div></section><section className="hhh-medication"><div className="hhh-section-inner"><p className="hhh-kicker">Medication</p><h2>Affordable medicine through your community pharmacy</h2><p>Indicative medicine prices; your exact treatment and costs are confirmed before you proceed.</p><div>{[['Flower','From £5.50 per gram'],['Oils','From £30 per 10ml bottle'],['Vape','From £49 per cartridge']].map(([name,price]) => <article key={name}><span><Leaf /></span><h3>{name}</h3><p>{price}</p></article>)}</div></div></section><section className="hhh-included hhh-section-inner"><div className="hhh-included__image"><img src={WELLBEING_IMAGE} alt="A couple enjoying a relaxed walk together" /></div><div><p className="hhh-kicker">What’s included in your care?</p><h2>Support that continues beyond your first appointment.</h2>{['Personalised treatment','Specialist care','Ongoing support','No waiting list'].map(item => <p className="hhh-included__item" key={item}><Check />{item}</p>)}<PublicLink href="/eligibility" className="hhh-button hhh-button--rust">Start your journey today</PublicLink></div></section><section className="hhh-timeline"><div className="hhh-section-inner"><p className="hhh-kicker">Your care timeline</p><h2>Your first months of care</h2><div>{[['Month 1','Initial consultation','£30'],['Month 2','Follow-up consultation','£30'],['Month 5','Quarterly check-up thereafter','£30']].map(([month,label,price]) => <article key={month}><span>{month}</span><h3>{label}</h3><strong>{price}</strong></article>)}</div></div></section></main></PageShell>;
}

function AboutPage() {
  return <PageShell><main id="main-content"><section className="hhh-about-hero"><img src={WELLBEING_IMAGE} alt="A couple enjoying a relaxed walk through a garden" /><div><p className="hhh-kicker">Our purpose</p><h1>Get back to doing the things you enjoy most.</h1></div></section><section className="hhh-about-copy hhh-section-inner"><p>Our mission is to make carefully considered, plant-based treatment options easier to explore through trusted local healthcare. Your care team can include specialist <strong>doctors</strong>, clinical <strong>pharmacists</strong> and <strong>nurses</strong>, all committed to treating you as an individual.</p><p>Every patient is different. Your clinical team works with you to understand your history, your priorities and whether a personalised cannabis-based treatment plan may be appropriate.</p><div><PublicLink href="/conditions" className="hhh-button hhh-button--outline">Explore conditions</PublicLink><PublicLink href="/pricing" className="hhh-button hhh-button--outline">View pricing</PublicLink></div></section><section className="hhh-planet"><div className="hhh-section-inner"><div><p className="hhh-kicker">Our commitment to the planet</p><h2>Care that looks<br />beyond today.</h2></div><div><p><strong>We believe healthcare should consider the wellbeing of people and the world around them.</strong></p><p>Future generations deserve a greener planet with better air quality.</p><p>For every CBPM prescription dispensed at a participating pharmacy, we support tree planting through Ecologi.</p><PublicLink href="/eligibility" className="hhh-button hhh-button--rust">Check your eligibility</PublicLink></div></div></section></main></PageShell>;
}
  ...
  return <PageShell><main id="main-content">
    ...
    <section className="hhh-contact hhh-section-inner">
      ...
      <div className="hhh-contact__layout">
        ...
        <article className="hhh-contact__card hhh-contact__card--green"><span className="hhh-contact__icon"><HeartPulse aria-hidden="true" /></span><p className="hhh-kicker">Health and eligibility</p><h3>Use the secure form</h3><p>Tell HHH about your circumstances, choose a convenient published pharmacy where available, or continue for manual allocation.</p><PublicLink href="/eligibility" className="hhh-button hhh-button--pale">Start eligibility check <ArrowRight /></PublicLink><small>Your application goes to HHH first. A pharmacy sees a patient record only after HHH confirms the referral.</small></article>
      </div>
    </section>
  </main></PageShell>;
}
...
function HowItWorksPage() { return <PageShell><main id="main-content">
  <InnerPageHero eyebrow="A supported route to care" title={<>From first questions<br />to ongoing support.</>} copy="HHH stays at the centre of your intake and referral, so a pharmacy receives your record only when the referral is ready." image={HERO_IMAGE} imageAlt="A pharmacist listening carefully during a private conversation">
    <PublicLink href="/eligibility" className="hhh-button hhh-button--rust">Check eligibility <ArrowRight /></PublicLink>
    <PublicLink href="/pricing" className="hhh-text-link">See pricing <ChevronRight /></PublicLink>
  </InnerPageHero>
  <section className="hhh-process hhh-section-inner"><header><p className="hhh-kicker">Your journey</p><h2>Four stages. One coordinated experience.</h2><p>A dedicated pharmacy link fixes your intended pharmacy; the main website lets you express a location preference. In both cases, HHH completes the intake first.</p></header><ol>{steps.map(step => <li key={step.number}><span className="hhh-process__number">{step.number}</span><span className="hhh-process__icon" aria-hidden="true">{step.icon}</span><div><h3>{step.title}</h3><p>{step.copy}</p></div></li>)}</ol></section>
...
export default function PublicSite() {
  ...
  useEffect(() => {
    ...
    const page = article ? { title: article.title, description: article.excerpt } : {
      '/': { title: 'Specialist care, closer to home', description: 'Explore personalised specialist care with support from participating community pharmacies.' },
