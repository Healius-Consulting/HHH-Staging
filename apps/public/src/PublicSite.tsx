...
const steps = [
  ...
  {
    image: SUPPORT_IMAGE,
    imageAlt: 'A care coordinator speaking with a patient by phone',
    icon: <HeartHandshake />,
    number: '04',
...
function HomePage() {
  return <PageShell><main id="main-content">
    <section className="hhh-hero hhh-reveal"><div className="hhh-section-inner hhh-hero__inner"><div className="hhh-hero__copy"><p className="hhh-kicker">Personalised healthcare</p><h1>Feel heard.<br />Find a way forward.</h1><p className="hhh-hero__lede">At Holistic Health Hub, we are dedicated to improving your health and well-being. We offer access to specialist therapies, including comprehensive medical cannabis (CBPM) treatment programmes for a variety of conditions, provided by specialist healthcare professionals.</p><p>Get access to evidence-based treatment and compassionate care through our network of trusted partnered pharmacies and specialist clinic.</p><div className="hhh-hero__actions"><PublicLink href="/eligibility" className="hhh-button hhh-button--rust">Check your eligibility <ArrowRight /></PublicLink><PublicLink href="/how-it-works" className="hhh-text-link">See how it works <ChevronRight /></PublicLink></div><div className="hhh-hero__assurance"><span><ShieldCheck /> Private and secure</span><span><Stethoscope /> Specialist-led</span><span><HeartHandshake /> Pharmacy connected</span></div></div><div className="hhh-hero__media"><img src={HERO_IMAGE} alt="A clinician listening to a patient in a private consultation room" fetchPriority="high" /><div><span><HeartPulse /></span><p><strong>Care built around you</strong><small>From first questions to ongoing support</small></p></div></div></div></section>
    <section className="hhh-journey hhh-reveal-block"><div className="hhh-section-inner"><p className="hhh-kicker">A clear route to care</p><h2>How it works in four simple steps</h2><div className="hhh-journey__grid">{steps.map((step, index) => <article key={step.number} style={{ '--stagger-index': index } as CSSProperties}><figure><img src={step.image} alt={step.imageAlt} /><figcaption>{step.number}</figcaption></figure><div><h3>{step.title}</h3><p>{step.copy}</p></div></article>)}</div></div></section>
    <section className="hhh-network hhh-reveal-block"><div className="hhh-section-inner"><p className="hhh-kicker">Who supports you</p><h2>HHH, a specialist clinic and your pharmacy, working together.</h2><div>{[['Holistic Health Hub','Reviews your eligibility, stays with you through intake and confirms the referral.'],['Specialist clinic','A doctor who specialises in your condition assesses you. Treatment decisions are clinical, not automatic.'],['Nominated pharmacy','Arranges payment, dispensing and delivery or collection once a prescription is issued.']].map(([title, copy], index) => <article key={title} style={{ '--stagger-index': index } as CSSProperties}><span>0{index + 1}</span><h3>{title}</h3><p>{copy}</p></article>)}</div></div></section>
    <section className="hhh-benefits hhh-section-inner hhh-reveal-block">{[{ icon: <Globe2 />, text: 'Online appointments that suit you' }, { icon: <Clock3 />, text: 'Personalised treatment options' }, { icon: <Video />, text: 'No GP referral required' }, { icon: <UserRoundCheck />, text: 'Access to specialist medical professionals' }, { icon: <HeartHandshake />, text: 'Dedicated patient support' }].map((item, index) => <div key={item.text} style={{ '--stagger-index': index } as CSSProperties}><span>{item.icon}</span><p>{item.text}</p></div>)}</section>
    <section className="hhh-condition-feature hhh-reveal-block"><div className="hhh-section-inner"><div className="hhh-condition-feature__top"><div><p className="hhh-kicker">Conditions we support</p><h2>Some of the conditions<br />we can help you with</h2></div><PublicLink className="hhh-button hhh-button--outline" href="/conditions">Which conditions can be treated?</PublicLink></div><div className="hhh-condition-card"><div className="hhh-condition-card__intro"><span><Activity /></span><h3>Pain</h3><p>For many of the 28 million people in the UK living with chronic pain, traditional painkillers like opioids aren’t always the answer. Holistic therapies such as medical cannabis offer alternative options.</p></div><div className="hhh-condition-card__body"><h4>How medical cannabis can help with pain</h4><p>Everybody has an endocannabinoid system (ECS) which plays a significant role in regulating pain, inflammation and other vital functions. Medical cannabis, which contains phytocannabinoids like THC and CBD, influences how the body responds to pain signals.</p><p className="hhh-condition-card__note">Pain-related conditions we can help you treat:</p><div className="hhh-tag-list">{['Arthritis','Back Pain','Chronic Pain','Cluster Headache','Complex Regional Pain Syndrome','Cancer-Related Pain','Ehlers-Danlos Syndromes (EDS)','Endometriosis','Fibromyalgia','Musculoskeletal Pain','Migraine','Neuropathic Pain','Palliative Care','Sciatica'].map(tag => <span key={tag}>{tag}</span>)}</div></div></div></div></section>
    <section className="hhh-key-benefits hhh-section-inner hhh-reveal-block"><p className="hhh-kicker">Key benefits</p><div>{['Appointments with pain specialists','Alternative to opioids','Anti-inflammatory properties'].map((item, index) => <article key={item}><span>0{index + 1}</span><h3>{item}</h3><ChevronRight /></article>)}</div><PublicLink href="/eligibility" className="hhh-button hhh-button--rust">Check your eligibility today <ArrowRight /></PublicLink></section>
    <section className="hhh-testimonials hhh-reveal-block"><div className="hhh-section-inner"><p className="hhh-kicker">Patient testimonials</p><div>{[['“I felt that I was listened to, and the different types of pain I was experiencing was understood and my treatment plan was tailored to suit my individual needs.”','Keasha'],['“It wasn’t until I saw my consultant that I felt properly listened to for the first time in years. The service I’ve received is second to none.”','Xavier'],['“Life with social anxiety and insomnia is horrendous. But my experience at the clinic has been amazing, they have been very understanding, a life saver.”','Kim']].map(([quote,name]) => <blockquote key={name}><p>{quote}</p><cite>{name}</cite></blockquote>)}</div></div></section>
    <section className="hhh-press hhh-reveal-block" aria-label="As seen in"><div className="hhh-section-inner"><p className="hhh-kicker">As seen in</p><div>{['GOV.UK','Sky News','The Guardian'].map(name => <span key={name}>{name}</span>)}</div></div></section>
    <section className="hhh-trust-band hhh-reveal-block"><div className="hhh-section-inner"><span><ShieldCheck /></span><div><p className="hhh-kicker">Thoughtful, responsible care</p><h2>Private treatment should still feel personal.</h2><p>Your eligibility check is only a starting point. A specialist clinician makes treatment decisions after an appropriate assessment, and your pharmacy remains part of the support around you.</p></div><PublicLink href="/about" className="hhh-button hhh-button--pale">Meet HHH</PublicLink></div></section>
    <section className="hhh-learn hhh-section-inner hhh-reveal-block"><p className="hhh-kicker">Learn</p><div className="hhh-post-grid">{posts.slice(0,3).map(post => <PostCard key={post.slug} post={post} />)}</div><PublicLink href="/blog" className="hhh-text-link">Read all articles <ArrowRight /></PublicLink></section>
  </main></PageShell>;
}
...
function InnerPageHero({ eyebrow, title, copy, image, imageAlt, children }: { eyebrow: string; title: ReactNode; copy: string; image?: string; imageAlt?: string; children?: ReactNode }) {
  return <section className={`hhh-inner-hero hhh-reveal${image ? ' hhh-inner-hero--with-image' : ''}`}><div className="hhh-section-inner"><div className="hhh-inner-hero__copy"><p className="hhh-kicker">{eyebrow}</p><h1>{title}</h1><p>{copy}</p>{children && <div className="hhh-inner-hero__actions">{children}</div>}</div>{image && <div className="hhh-inner-hero__media"><img src={image} alt={imageAlt ?? ''} /></div>}</div></section>;
}

function PageCta({ kicker, title, copy, href, label }: { kicker: string; title: string; copy: string; href: string; label: string }) {
  return <section className="hhh-page-cta hhh-reveal-block"><div className="hhh-section-inner"><div><p className="hhh-kicker">{kicker}</p><h2>{title}</h2><p>{copy}</p></div><PublicLink href={href} className="hhh-button hhh-button--pale">{label}</PublicLink></div></section>;
}
...
function ConditionsPage() {
  return <PageShell><main id="main-content">
    <InnerPageHero eyebrow="Conditions we support" title={<>Conditions that can be treated<br />with medical cannabis (CBPM)</>} copy="If you have tried two therapies or treatments for these conditions, you may be eligible for referral. A specialist clinician decides whether treatment is appropriate.">
    ...
    <section className="hhh-condition-groups hhh-section-inner hhh-reveal-block">{conditionGroups.map((group, index) => <article key={group.title} style={{ '--stagger-index': index } as CSSProperties}><span>{group.icon}</span><h2>{group.title}</h2><ul>{group.items.map(item => <li key={item}><Check />{item}</li>)}</ul><PublicLink href="/eligibility">Check eligibility <ArrowRight /></PublicLink></article>)}</section>
    <PageCta kicker="Next question" title="What is the cost of treatment?" copy="Consultation fees are published clearly, and medicine prices are confirmed before you proceed." href="/pricing" label="View pricing" />
  </main></PageShell>;
}

function PricingPage() {
  return <PageShell><main id="main-content">
    <section className="hhh-pricing-head hhh-section-inner hhh-reveal"><p className="hhh-kicker">Simple, transparent pricing</p><h1>Get the personalised care you deserve, at affordable prices.</h1><p>All appointments are with a consultant physician who specialises in your condition.</p><div className="hhh-price-grid">{[['Initial consultation','£30'],['Follow-up consultation','£30'],['Quarterly check-up','£30']].map(([label,price], index) => <article key={label} style={{ '--stagger-index': index } as CSSProperties}><span>0{index + 1}</span><h2>{label}</h2><strong>{price}</strong><small>Consultation fee</small></article>)}</div></section>
    <section className="hhh-medication hhh-reveal-block"><div className="hhh-section-inner"><p className="hhh-kicker">Medication</p><h2>Affordable medicine through your community pharmacy</h2><p>Indicative medicine prices; your exact treatment and costs are confirmed before you proceed.</p><div>{[['Flower','From £5.50 per gram'],['Oils','From £30 per 10ml bottle'],['Vape','From £49 per cartridge']].map(([name,price], index) => <article key={name} style={{ '--stagger-index': index } as CSSProperties}><span><Leaf /></span><h3>{name}</h3><p>{price}</p></article>)}</div></div></section>
    <section className="hhh-included hhh-section-inner hhh-reveal-block"><div className="hhh-included__image"><img src={PHARMACY_IMAGE} alt="A community pharmacist speaking with a patient at the dispensary counter" /></div><div><p className="hhh-kicker">What’s included in your care?</p><h2>Support that continues beyond your first appointment.</h2>{['Personalised treatment','Specialist care','Ongoing support','No waiting list'].map(item => <p className="hhh-included__item" key={item}><Check />{item}</p>)}<PublicLink href="/eligibility" className="hhh-button hhh-button--rust">Start your journey today</PublicLink></div></section>
    <section className="hhh-timeline hhh-reveal-block"><div className="hhh-section-inner"><p className="hhh-kicker">Your care timeline</p><h2>Your first months of care</h2><div>{[['Month 1','Initial consultation','£30'],['Month 2','Follow-up consultation','£30'],['Month 5','Quarterly check-up thereafter','£30']].map(([month,label,price], index) => <article key={month} style={{ '--stagger-index': index } as CSSProperties}><span>{month}</span><h3>{label}</h3><strong>{price}</strong></article>)}</div></div></section>
  </main></PageShell>;
}

function AboutPage() {
  return <PageShell><main id="main-content">
    <section className="hhh-about-hero hhh-reveal"><img src={WELLBEING_IMAGE} alt="A couple walking together through a garden" /><div className="hhh-about-hero__copy"><p className="hhh-kicker">Our purpose</p><h1>Get back to doing the things you enjoy most.</h1></div></section>
    <section className="hhh-about-copy hhh-section-inner hhh-reveal-block"><p>Our mission is to provide holistic plant-based treatment options to those in need. Your team can include specialist <strong>doctors</strong>, clinical <strong>pharmacists</strong> and <strong>nurses</strong>, all committed to providing personalised care to each patient.</p><p>They understand that every patient is unique, and will work closely with you to develop a medical cannabis treatment plan that is tailored to your specific needs, where that is clinically appropriate.</p><div><PublicLink href="/conditions" className="hhh-button hhh-button--outline">Explore conditions</PublicLink><PublicLink href="/pricing" className="hhh-button hhh-button--outline">View pricing</PublicLink></div></section>
    <section className="hhh-team hhh-reveal-block"><div className="hhh-section-inner"><p className="hhh-kicker">Your care team</p><h2>Specialists, pharmacists and nurses, working around you.</h2><div>{[{ icon: <Stethoscope />, title: 'Specialist doctors', copy: 'A doctor who specialises in your condition assesses you and decides whether a prescription is appropriate.' }, { icon: <PackageCheck />, title: 'Clinical pharmacists', copy: 'Your nominated pharmacy supports payment, dispensing, delivery or collection after a prescription is issued.' }, { icon: <HeartPulse />, title: 'Nurses and support', copy: 'Personalised follow-up sits alongside the clinic and pharmacy, so care does not stop after the first appointment.' }].map((item, index) => <article key={item.title} style={{ '--stagger-index': index } as CSSProperties}><span>{item.icon}</span><h3>{item.title}</h3><p>{item.copy}</p></article>)}</div></div></section>
    <section className="hhh-planet hhh-reveal-block"><div className="hhh-section-inner"><div><p className="hhh-kicker">Our commitment to the planet</p><h2>Care that looks<br />beyond today.</h2></div><div><p><strong>We believe it is our collective duty to preserve the planet and the various forms of life that live on it.</strong></p><p>Future generations deserve a greener planet with better air quality.</p><p>Cannabis itself is a carbon sequester, meaning it takes in more CO2 than it produces. That is not enough on its own, so for every CBPM prescription dispensed at a participating pharmacy, we support tree planting through Ecologi.</p><PublicLink href="/eligibility" className="hhh-button hhh-button--rust">Check your eligibility</PublicLink></div></div></section>
  </main></PageShell>;
}
...
function FaqPage() {
  return <PageShell><main id="main-content">
    ...
    <PageCta kicker="Ready when you are" title="Take the first step securely." copy="Your eligibility application is reviewed by HHH before any patient record is activated for a pharmacy." href="/eligibility" label="Start eligibility check" />
  </main></PageShell>;
}

function ContactPage() {
  return <PageShell><main id="main-content">
    <InnerPageHero eyebrow="We’re here to help" title={<>Start the right<br />conversation.</>} copy="Choose the secure eligibility route for anything about your health, or email our team with a general service question.">
      <PublicLink href="/eligibility" className="hhh-button hhh-button--rust">Start eligibility check <ArrowRight /></PublicLink>
    </InnerPageHero>
    <section className="hhh-contact hhh-section-inner hhh-reveal-block">
      ...
      <div className="hhh-contact__layout">
        <article className="hhh-contact__card" style={{ '--stagger-index': 0 } as CSSProperties}><span className="hhh-contact__icon"><Mail aria-hidden="true" /></span><p className="hhh-kicker">General questions</p><h3>Email the HHH team</h3><p>For questions about our service, pricing or how the journey works.</p><a className="hhh-contact__email" href="mailto:info@holistichealthhub.live">info@holistichealthhub.live</a><div className="hhh-contact__notice"><ShieldCheck aria-hidden="true" /><p><strong>Keep health information secure.</strong> Please do not send health, prescription or patient details by email.</p></div></article>
        <article className="hhh-contact__card hhh-contact__card--green" style={{ '--stagger-index': 1 } as CSSProperties}><span className="hhh-contact__icon"><HeartPulse aria-hidden="true" /></span><p className="hhh-kicker">Health and eligibility</p><h3>Use the secure form</h3><p>Tell HHH about your circumstances, choose a convenient participating pharmacy where available, or continue with Holistic Health Hub Allocation.</p><PublicLink href="/eligibility" className="hhh-button hhh-button--pale">Start eligibility check <ArrowRight /></PublicLink><small>Your application goes to HHH first. A pharmacy sees a patient record only after HHH confirms the referral.</small></article>
      </div>
    </section>
  </main></PageShell>;
}
...
function HowItWorksPage() { return <PageShell><main id="main-content">
  <InnerPageHero eyebrow="A supported route to care" title={<>From first questions<br />to ongoing support.</>} copy="HHH stays at the centre of your intake and referral. A specialist clinic assesses you, and a pharmacy receives your record only when the referral is ready.">
    <PublicLink href="/eligibility" className="hhh-button hhh-button--rust">Check eligibility <ArrowRight /></PublicLink>
    <PublicLink href="/pricing" className="hhh-text-link">See pricing <ChevronRight /></PublicLink>
  </InnerPageHero>
  <section className="hhh-process-story">{steps.map((step, index) => <article className={`hhh-process-story__row hhh-reveal-block${index % 2 ? ' is-reverse' : ''}`} key={step.number}><figure><div className="hhh-process-story__frame"><img src={step.image} alt={step.imageAlt} /></div><figcaption>{step.number}</figcaption></figure><div><p className="hhh-kicker">Step {step.number}</p><h2>{step.title}</h2><p>{step.copy}</p></div></article>)}</section>
  <section className="hhh-visibility-band hhh-reveal-block"><div className="hhh-section-inner"><div><p className="hhh-kicker">Before HHH refers you</p><h2>Your application stays with HHH.</h2><p>HHH can record follow-up needs, check your route and confirm the destination. A pharmacy does not review new eligibility applications.</p></div><div><p className="hhh-kicker">After HHH confirms</p><h2>Your pharmacy record is activated.</h2><p>The confirmed pharmacy can then support the operational parts of your care, including payment, dispensing, delivery or collection.</p></div></div></section>
  <section className="hhh-how-next hhh-section-inner hhh-reveal-block"><span><ShieldCheck aria-hidden="true" /></span><div><p className="hhh-kicker">Private by design</p><h2>Ready to see whether you may be eligible?</h2><p>The secure form keeps your health information out of emails, page URLs and browser storage.</p></div><PublicLink href="/eligibility" className="hhh-button hhh-button--rust">Begin securely</PublicLink></section>
</main></PageShell>; }
