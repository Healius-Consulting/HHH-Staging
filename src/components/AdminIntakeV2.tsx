...
export default function AdminIntakeV2() {
  ...
  return <div className="admin-v2-intake">
    <section className="admin-v2-intake__boundary" aria-label="Patient intake security boundary">
      <ShieldCheck size={19} />
      <span><strong>HHH intake workspace</strong><small>HHH can review every enquiry as it arrives. A live pharmacy sees the current destination enquiry in Patients until referral. Moving assignment transfers visibility to the new pharmacy. Training workspaces and pharmacies that have not gone live cannot see enquiries.</small></span>
    ...
    <div className="admin-v2-intake__workspace">
      ...
      <main className="admin-v2-intake__detail">
        {!selected ? <div className="admin-v2-intake__placeholder"><ClipboardList size={28} /><h2>Select a patient</h2><p>Choose someone from the protected queue to review their form, update the intended pharmacy, and complete the referral.</p></div> : detailLoading || !detail ? <div className="empty-state"><LoaderCircle className="spin" size={22} /> Loading full authorised form…</div> : <>
          ...
          <section className="admin-v2-case__notice"><ShieldCheck size={18} /><span><strong>Enquiries stay visible until referral</strong><small>Only the current destination pharmacy can see this person as an enquiry. Completing referral marks them referred for that pharmacy.</small></span></section>
