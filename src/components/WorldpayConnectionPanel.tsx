...
export default function WorldpayConnectionPanel({
  ...
  const refresh = useCallback(async () => {
    ...
    setError(null);
    try {
      applyStatus(await getWorldpayConnectionStatus(organisationId));
    } catch (refreshError) {
  ...
  const connect = async () => {
    ...
    try {
      applyStatus(await connectWorldpayPharmacy({
        ...
        customisationId: form.customisationId.trim() || undefined,
      }));
    } catch (connectError) {
  ...
  const saveBranding = async () => {
    ...
    setError(null);
    try {
      applyStatus(await updateWorldpayBranding({
        organisationId,
        customisationId: form.customisationId.trim() || undefined,
      }));
    } catch (brandingError) {
      setError(brandingError instanceof Error ? brandingError.message : 'The hosted payment page customisation ID could not be saved.');
    } finally {
      setBusy(false);
    }
  };
  ...
  return (
    <section className="worldpay-connect-panel">
      ...
      {status?.connected ? (
        <div className="worldpay-connect-panel__branding">
          <p>
            <strong>Hosted page branding</strong>
            <small>{status.brandingConfigured ? 'A Worldpay customisation ID is stored for this pharmacy’s hosted checkout.' : 'Add the Worldpay customisation ID so the hosted page can use this pharmacy’s logo.'}</small>
          </p>
          <label className="worldpay-connect-panel__wide">
            <span>Customisation ID</span>
            <input
              className="input"
              autoComplete="off"
              placeholder="From Worldpay HPP Designer"
              value={form.customisationId}
              onChange={event => setForm(current => ({ ...current, customisationId: event.target.value }))}
            />
          </label>
          <p className="worldpay-connect-panel__hint">Copy this from Worldpay Dashboard → Developer Tools → HPP Designer. It sits next to the channel dropdown. Leave blank and save to remove branding.</p>
          <button type="button" className="btn btn-sm" disabled={busy} onClick={() => void saveBranding()}>Save branding</button>
        </div>
      ) : null}
      ...
      {showCredentialForm && (
        <div className="worldpay-connect-panel__fields">
          ...
          {!status?.connected ? <p className="worldpay-connect-panel__hint worldpay-connect-panel__wide">Optional. Copy this from Worldpay Dashboard → Developer Tools → HPP Designer, next to the channel dropdown. This is what applies the pharmacy logo on the hosted payment page.</p> : null}
      ...
      {status?.connected ? (
        ...
        <button type="button" className="btn btn-sm" disabled={busy} onClick={() => void refresh()}><RefreshCw size={13} className={busy ? 'spin' : ''} /> Refresh connection status</button>
      ) : null}
