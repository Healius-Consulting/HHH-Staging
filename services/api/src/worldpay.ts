...
export type WorldpayCredential = { username: string; password: string; entityId: string; webhookSecret: string };
...
export async function createHostedPaymentSession(organisationId: string, input: { transactionReference: string; amountPence: number; currency: 'GBP'; successUrl: string; cancelUrl: string; statementNarrative: string }) {
...
export async function reconcileWorldpayPayment(organisationId: string, transactionReference: string) {
  ...
  const response = await fetch(new URL(`/payments/${encodeURIComponent(transactionReference)}`, config.WORLDPAY_VERIFY_BASE_URL), {
    headers: { Authorization: worldpayAuthorization(credential), Accept: 'application/json' },
  });
