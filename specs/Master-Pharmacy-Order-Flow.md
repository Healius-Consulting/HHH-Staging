...
- A paid patient is never charged more without a recorded top-up. A patient-price drop is either refunded or, if the pharmacy chooses to continue, recorded as extra dispensing fee — never kept silently.
...
3. **Placement:** exact per-prescription requote and one PO per prescription. Out-of-stock holds. Supplier-cost-only increases proceed only when fixed patient revenue plus allocated order fee retains a 15% line margin; otherwise the line is held. Patient-price **up**: absorb or payment link for the difference. Patient-price **down**: refund the difference, or continue and record the drop as dispensing fee.
